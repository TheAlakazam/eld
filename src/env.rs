...
impl<'parent> Env<'parent> {
    ...
    pub(crate) fn get_binding_value(&self, name: &str) -> Result<Val, String> {
        self.get_binding_value_withou_error_msg(name)
            .ok_or_else(|| format!("binding with name '{}' does not exist", name))
    }

    fn get_binding_value_withou_error_msg(&self, name: &str) -> Option<Val> {
        self.bindings.get(name).cloned().or_else(|| {
            self.parent
                .and_then(|parent| parent.get_binding_value_withou_error_msg(name))
        })
    }
